...
import {
    ...
    type SalaryExpenseReportResponse,
...
export default function SalaryExpenseReportPage() {
    ...
    const [report, setReport] = useState<SalaryExpenseReportResponse | null>(null);
    ...
    // Load staff and payment accounts for dropdown
    useEffect(() => {
        getStaff({ limit: 100 })
            .then((res) => setStaffList(res.data.items))
            .catch(() => {});
    ...
    useEffect(() => {
        ...
        async function load() {
            try {
                setIsLoading(true);
                setError(null);
                const data = await getSalaryExpenseReport({
                    fromDate,
                    toDate,
                    page,
                    limit,
                ...
                if (!cancelled) {
                    setError("Failed to load salary expenses report.");
                    setReport(null);
                }
            } finally {
    ...
    }, [fromDate, toDate, page, limit, search, staffFilter, paymentMethodFilter]);
    ...
    const hasActiveFilters = useMemo(
        () => staffFilter !== "all" || paymentMethodFilter !== "ALL" || search.length > 0,
        [staffFilter, paymentMethodFilter, search]
    );
        ...
        <section className="w-full space-y-4 px-3 py-4 sm:space-y-6 sm:px-6">
                    ...
                    <div className="flex min-w-0 items-center gap-3">
                        ...
                        <div className="min-w-0">
                            <h1 className="truncate text-xl font-semibold tracking-tight text-slate-950 dark:text-white sm:text-2xl">
                                Salary & Staff Payouts
                            </h1>
                            <p className="truncate text-sm text-slate-500 dark:text-slate-400">
                                Track all staff salary payouts and payment accounts for the selected period.
                            </p>
                        </div>
                    </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800/60 dark:bg-slate-900/50">
                    ...
                    <p className="mt-2 text-2xl font-bold tracking-tight text-slate-950 dark:text-white">
                        {formatCurrency(summary.count > 0 ? summary.totalSalaryPaid / summary.count : 0)}
                    </p>
                    <p className="mt-1 text-xs text-slate-400">
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div className="relative w-full md:w-80">
                    ...
                    <Input
                        placeholder="Search staff, emp code, voucher..."
                        value={searchInput}
                ...
                <div className="flex flex-wrap items-center gap-2">
                    ...
                    >
                        <SelectTrigger className="h-10 w-full sm:w-[180px] rounded-lg border-slate-300 bg-white text-slate-900 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100">
                    ...
                    <Table className="w-full min-w-[850px] text-sm">
                        <TableHeader>
                            <TableRow className="bg-[#556043] hover:bg-[#556043] dark:bg-background dark:hover:bg-background border-none">
                                ...
                                <TableHead className="px-4 py-3 text-white font-semibold whitespace-nowrap">Staff Name & Code</TableHead>
                                <TableHead className="px-4 py-3 text-white font-semibold whitespace-nowrap">Category / Type</TableHead>
                                ...
                                <TableRow>
                                    <TableCell colSpan={8} className="h-44 text-center">
                                        <div className="flex flex-col items-center justify-center gap-2 text-slate-500">
                                            ...
                                            <p className="text-sm">Loading salary expenses report...</p>
                                        </div>
                                    </TableCell>
                                </TableRow>
                            ) : error ? (
                                ...
                                <TableRow>
                                    <TableCell colSpan={8} className="h-44 text-center text-slate-500">
                                        <div className="flex flex-col items-center justify-center gap-2">
                                            ...
                                            <p className="text-sm">No salary payouts recorded for this range.</p>
                                        </div>
                                    </TableCell>
                                </TableRow>
                            ) : (
                                items.map((item) => (
                                    <TableRow key={item.expenseId} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/40">
                                        ...
                                        <TableCell className="px-4 py-3 font-medium text-slate-950 dark:text-white whitespace-nowrap">
                                            {item.staffName}
                                            <span className="ml-1.5 text-xs text-slate-400">({item.employeeCode})</span>
                                        </TableCell>
                                        <TableCell className="px-4 py-3 text-slate-700 dark:text-slate-300 whitespace-nowrap">
                                            {item.subCategoryName !== "-" ? item.subCategoryName : item.categoryName}
                                        </TableCell>
