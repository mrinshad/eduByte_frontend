...
export default function FeeTypeReportPage({ params }: { params: Promise<{ id: string }> }) {
    const resolvedParams = use(params);
    const chargeTypeId = resolvedParams.id;
    ...
    useEffect(() => {
        ...
        async function load() {
            try {
                setIsLoading(true);
                setError(null);
                const data = await getFeeTypeCollectionReport(chargeTypeId, {
                    fromDate,
                    toDate,
                    page,
                    limit,
                ...
                if (!cancelled) {
                    setError("Failed to load fee collection report.");
                    setReport(null);
                }
            } finally {
                    ...
                    <Table className="w-full min-w-[850px] text-sm">
                        ...
                        <TableBody className="divide-y divide-slate-100 dark:divide-slate-800/50">
                            {isLoading ? (
                                <TableRow>
                                    <TableCell colSpan={8} className="h-44 text-center">
                                        <div className="flex flex-col items-center justify-center gap-2 text-slate-500">
                                            ...
                                            <p className="text-sm">Loading fee collection report...</p>
                                        </div>
                                    </TableCell>
                                </TableRow>
                            ) : error ? (
                                ...
                                items.map((item) => (
                                    <TableRow key={item.allocationId} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/40">
                                        ...
                                        <TableCell className="px-4 py-3 font-medium text-slate-950 dark:text-white whitespace-nowrap">
                                            {item.studentName}
                                            <span className="ml-1.5 text-xs text-slate-400">({item.admissionNumber})</span>
                                        </TableCell>
                                        ...
                                        <TableCell className="px-4 py-3 text-right whitespace-nowrap">
                                            <Button
                                                ...
                                                onClick={() => router.push(`/print/fee-collection/${item.transactionId}`)}
                                                title="Print Receipt"
                                            >
