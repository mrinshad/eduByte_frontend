...
import {
    getTransportationExpenseReport,
...
export default function TransportationExpensesReportPage() {
    ...
    const [report, setReport] = useState<TransportationExpenseReportResponse | null>(null);
    ...
    useEffect(() => {
        ...
        async function load() {
            try {
                setIsLoading(true);
                setError(null);
                const data = await getTransportationExpenseReport({
                    fromDate,
                    toDate,
                    page,
                    limit,
                ...
                if (!cancelled) {
                    setError("Failed to load transportation expenses report.");
                    setReport(null);
                }
            } finally {
    ...
    }, [fromDate, toDate, page, limit, search, vehicleFilter, subCategoryFilter, paymentMethodFilter]);
    ...
    const hasActiveFilters = useMemo(
        () => vehicleFilter !== "all" || subCategoryFilter !== "all" || paymentMethodFilter !== "ALL" || search.length > 0,
        [vehicleFilter, subCategoryFilter, paymentMethodFilter, search]
    );
        ...
        <section className="w-full space-y-4 px-3 py-4 sm:space-y-6 sm:px-6">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <div className="rounded-2xl border border-slate-200 bg-[#556043] p-4 text-white shadow-sm dark:border-slate-800/60 dark:bg-slate-900/50">
                    ...
                    <p className="mt-2 text-2xl font-bold tracking-tight text-white">
                        {formatCurrency(summary.totalTransportExpense)}
                    </p>
                ...
                <div className="rounded-2xl border border-amber-200 bg-amber-50/70 p-4 shadow-sm dark:border-amber-500/30 dark:bg-amber-500/10">
                    ...
                    <p className="mt-2 text-2xl font-bold tracking-tight text-amber-950 dark:text-amber-100">
                ...
                <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-800/60 dark:bg-slate-900/50">
                    ...
                    <p className="mt-2 text-2xl font-bold tracking-tight text-slate-950 dark:text-white">
                        {formatCurrency(summary.partsTotal)}
                    </p>
            ...
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div className="relative w-full md:w-80">
                    ...
                    <Input
                        placeholder="Search vehicle, driver, voucher..."
                        value={searchInput}
                                ...
                                items.map((item) => (
                                    <TableRow key={item.expenseId} className="hover:bg-slate-50/50 dark:hover:bg-slate-900/40">
                                        ...
                                        <TableCell className="px-4 py-3 font-medium text-slate-950 dark:text-white whitespace-nowrap">
                                            {item.vehicleName}
                                            <span className="ml-1.5 text-xs text-slate-400">({item.vehicleNumber})</span>
                                        </TableCell>
