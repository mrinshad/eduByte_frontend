...
function SummaryPaymentSkeleton() {
  return (
    <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
      <div className="rounded-lg border border-slate-200 p-4 dark:border-slate-800/50 space-y-3">
...
export default function Page() {
  ...
  const selectedPositiveCharges = useMemo(
    () =>
      charges.filter(
        (charge) =>
          charge.finalAmount > 0 && chargeKey(charge.id) in selected && selected[chargeKey(charge.id)] > 0
      ),
    [charges, selected]
  )
  ...
  const groupedCharges = useMemo(() => {
    ...
    for (const charge of charges) {
      ...
      const periodKey = hasPeriod
        ? (charge.periodYear as number) * 100 + (charge.periodMonth as number)
        : -1
      const label = hasPeriod
        ? new Date(charge.periodYear as number, (charge.periodMonth as number) - 1, 1).toLocaleString(
            "default",
              ...
              {successInfo && (
                <div className="flex items-center gap-3 rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-emerald-800 dark:border-emerald-500/30 dark:bg-emerald-500/10 dark:text-emerald-400">
                  ...
                  <p className="text-sm font-medium">
                    Payment collected successfully. Transaction {successInfo.transactionNumber} for{" "}
                    {formatCurrency(successInfo.totalAmount)}.
                  </p>
                </div>
              )}
                ...
                <div className="rounded-lg border border-slate-200 p-4 dark:border-slate-800/50">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-slate-950 dark:text-slate-100">
                        {formatCurrency(positiveOutstanding)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                  ...
                  <p className="mt-3 text-xs text-slate-500 dark:text-slate-400">
                    You can adjust each amount above — the accountant may collect a partial
                    payment.
                  </p>
                </div>
                ...
                <div className="rounded-lg border border-slate-200 p-4 dark:border-slate-800/50">
                  ...
                  {paymentAccounts.length === 0 ? (
                    <p className="mb-3 text-xs font-medium text-amber-600 dark:text-amber-400">
                      No active payment method accounts found. Seed or create payment accounts
                      first.
                    </p>
                  ) : null}
                  ...
                  {selectedZeroChargeIds.length > 0 && payableOutstanding === 0 && (
                    <p className="mt-2 text-xs font-medium text-emerald-600">
                      {selectedZeroChargeIds.length} zero-amount charge(s) selected. They will be
                      marked as paid without payment.
                    </p>
                  )}
